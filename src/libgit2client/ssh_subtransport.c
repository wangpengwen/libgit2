...
static int start_ssh(
	...
	const char *sshpath)
{
	const char *args_array[4];
	const char *env_array[] = { "GIT_DIR=" };

	git_process_options process_opts = CLI_PROCESS_OPTIONS_INIT;
	git_strarray args = { (char **)args_array, 4 };
	git_strarray env = { (char **)env_array, 1 };
	...
	args_array[0] = "/usr/bin/ssh";
	args_array[1] = userhost.ptr;
	args_array[2] = command;
	args_array[3] = url.path;

	if ((error = git_process_new(&transport->process, &args, &env, &process_opts)) < 0 ||
